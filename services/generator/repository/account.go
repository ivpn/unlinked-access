...
func (d *Database) GetAccounts() ([]*model.Account, error) {
	...
	if d.Cfg.Service.Mock {
		...
		err = d.Client.
			Where("is_new = ?", false).
			Where("EXISTS (SELECT 1 FROM services WHERE services.accounting_id = accounts.accounting_id AND services.s_active = true AND accounts.active_until > NOW() - INTERVAL 14 DAY)").
			Find(&accounts).Error
